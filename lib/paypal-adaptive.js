...
Paypal.prototype.genSignature = function (username, password, token, token_secret, url, timestamp) {
  ...
  var key = [
    this.paypal_encode(password),
    this.paypal_encode(token_secret)
  ...
  var baseString = [
    'POST',
    this.paypal_encode(url),
    this.paypal_encode(sortedQueryString)
  ].join('&').replace(/%([0-9A-F][0-9A-F])/g, function (v) {
    return v.toLowerCase()
  });
...
Paypal.prototype.paypalEncode = function (str) {
  return encodeURIComponent(str).replace('.', '%2E').replace('-', '%2D');
}
